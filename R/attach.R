pkgs_unloaded <- function(cluster) {
  pkgs <- myverse_packages(FALSE, cluster)
...
#' @export
myverse_attach <- function(cluster = NULL) {
  to_load <- pkgs_unloaded(cluster)
  if (length(to_load) == 0)
  ...
  msg(
    cli::rule(
      ...
      right = paste0("myverse ", package_version("myverse"))
    ),
    startup = TRUE
  )
